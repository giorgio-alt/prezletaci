...
export const baseWebsiteContentItems: WebsiteContentItem[] = [
  ...
  { id: "page-plans", title: programContent.title, pageType: "Programová stránka", section: "Plány", purpose: programContent.perex, pillar: "Plány", status: "Připraveno k publikaci", priority: "Kritická", owner: "PM + Copy", deadline: "14. 8. 2026", sourceLinks: programContent.sourceLinks, draftLink: programContent.markdownPath, notes: `Deset programových okruhů včetně komunitního života a sociálních vazeb · webová adresa ${programContent.futureWebPath}.`, blockers: [], readiness: 100, checklist: [{ label: "Úvodní perex", available: true }, { label: "Deset programových okruhů", available: true }, { label: "Sociální a mezigenerační vazby", available: true }, { label: "Navazující témata", available: true }] },
  ...
  ...articleContent.map((article): WebsiteContentItem => {
    ...
    notes: `Primární obrázek: ${article.primaryImage}. Doplňkové obrázky: ${article.galleryImages.join(" · ") || "žádné"}.`,
    blockers: [],
    readiness: 100,
    checklist: [
...
export function buildWebBriefMarkdown() {
  ...
  const articles = articleContent.map((article) => `### ${article.title}\n\n- Markdown: ${article.markdownPath}\n- Primární obrázek: ${article.primaryImage}\n- Doplňkové obrázky: ${article.galleryImages.join(" · ") || "Žádné"}\n- Související projekty: ${article.projectIds.join(", ") || "Čeká"}\n- SoMe posty: ${article.socialPostIds.join(", ") || "Čeká"}`).join("\n\n");
...
export const FEEDBACK_GUIDELINES_MARKDOWN = `## Zapracování připomínek
