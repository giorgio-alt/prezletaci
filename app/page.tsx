...
import { articleContent, articleContentBySlug, articleToMarkdown, getArticleImageDescription, type ArticleContent } from "./article-content";
import {
...
type ProjectOperations = Pick<Project, "id" | "owner" | "evidence" | "risk" | "argument" | "next" | "history">;

const projectOperations: ProjectOperations[] = [
  { id: 1, owner: "Obec", evidence: "Doplnit fotografie a umístění", risk: "Bez fotek působí výsledek abstraktně.", argument: "Jde o ověřený formát, na který lze navázat dalšími lokalitami.", next: "Vybrat další místa a připravit stavbu.", history: "Realizovány tři celky; pokračování je součástí programu 2026–2030." },
  ...
  { id: 14, owner: "Obec + developer", evidence: "Dohoda s developerem", risk: "Nákladnost a závislost na developerské výstavbě.", argument: "Vodojem řeší existující poklesy tlaku i budoucí růst obce.", next: "Doložit kapacitní výpočet a smluvní zajištění.", history: "Potřeba identifikována, investorem má být developer." },
  { id: 15, owner: "Obec + developer", evidence: "Smluvní rámec s developerem", risk: "Složitý technický projekt a růst obce.", argument: "Rozšíření existujícího řešení drží infrastrukturu pod kontrolou obce.", next: "Zpracovat veřejně srozumitelný kapacitní přehled.", history: "Projekt je ve fázi plánování; investorem má být developer." },
  { id: 16, owner: "Obec", evidence: "Studie rekonstrukce", risk: "Rozpočet, etapizace a budoucí provozní náklady.", argument: "Hotová studie umožňuje otevřeně porovnat rozsah, cenu a etapy.", next: "Zveřejnit studii a varianty financování.", history: "Studie je hotová; projekt čeká na další přípravu." },
  { id: 17, owner: "Obec", evidence: "Demografická data a územní podklady", risk: "Termín musí odpovídat reálnému růstu počtu dětí.", argument: "Kapacita se má připravovat podle dat, ne až po vzniku akutního problému.", next: "Aktualizovat demografický výhled.", history: "Záměr je navázán na rozvoj lokality a potřeby obce." },
  { id: 18, owner: "Obec + developer", evidence: "Smluvní závazek a prostorový program", risk: "Vnímání jako nákladná budova místo služby občanům.", argument: "Nové prostory reagují na personální a agendový růst obce.", next: "Ukázat kapacitní potřebu a zdroj financování.", history: "Záměr je smluvně spojen s developerskou výstavbou." },
];

const projectOperationsById = new Map(projectOperations.map((project) => [project.id, project]));
...
const initialProjects: Project[] = publicProjectContent.map((project) => {
  ...
  return {
    ...
    area: project.area,
    owner: operations?.owner ?? "Obec a partneři",
    summary: project.summary,
    details: project.details,
    milestones: project.milestones,
    publicSources: project.sourceUrls ?? [],
    evidence: operations?.evidence ?? (project.sourceUrls?.map((source) => source.label).join(", ") || (project.image ? "Fotografie projektu" : "Aktuální stav projektu")),
    risk: operations?.risk ?? "",
    argument: operations?.argument ?? project.summary,
    next: operations?.next ?? project.details.at(-1) ?? project.summary,
    history: operations?.history ?? project.details.join(" "),
  };
});
...
export default function Home() {
      ...
      <section className="project-grid">
        {filteredProjects.map((project) => (
          <button className="project-card glass-card" key={project.id} onClick={() => setSelectedProject(project)}>
            ...
            {project.image ? <div className="project-image"><Image src={project.image} alt={project.imageAlt || `Obrazový podklad projektu ${project.title}`} fill sizes="(max-width: 760px) 100vw, (max-width: 1100px) 50vw, 33vw" unoptimized />{project.imageKind && project.imageKind !== "fotografie" && <small className="project-image-label">{project.imageKind === "ilustrace" ? "Ilustrace" : "Vizualizace"}</small>}</div> : <div className="project-image-placeholder"><span>{project.area.slice(0, 1)}</span></div>}
  ...
  const renderArticleLibrary = () => (
    <section className="article-library section-stack" aria-label="Centrální seznam článků">
      <div className="article-library-hero glass-card">
        <div><span className="eyebrow">Article Library</span><h2>Všechny články na jednom místě</h2><p>Odsud otevřeš webový text, vezmeš text pro sociální sítě a připravíš sdílení.</p></div>
        <div className="article-library-stats"><strong>{articleContent.length}</strong><span>připravených článků</span></div>
      </div>
  ...
  return (
    <div className={`campaign-app theme-${theme}`}>
      ...
      {selectedArticle && <div className="modal-backdrop" onMouseDown={() => setSelectedArticle(null)}><section className="detail-modal article-library-modal article-readable-card" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelectedArticle(null)}>×</button><h2>{selectedArticle.title}</h2><p className="article-perex">{selectedArticle.perex}</p><div className="article-modal-actions"><button className="primary-button" onClick={() => shareArticle(selectedArticle)}>Sdílet</button><a className="secondary-button" href={articlePublicPath(selectedArticle)} target="_blank" rel="noreferrer">Otevřít na webu</a><a className="secondary-button" href={articleMarkdownUrl(selectedArticle)} target="_blank" rel="noreferrer">Otevřít Markdown</a><button className="secondary-button" onClick={() => copyToClipboard(articleFullText(selectedArticle), "Celý článek")}>Kopírovat celý článek</button><button className="secondary-button" onClick={() => copyToClipboard(selectedArticle.socialCopy, "SoMe text článku")}>Kopírovat SoMe text</button></div><div className="article-modal-media"><Image src={selectedArticle.primaryImage} alt={getArticleImageDescription(selectedArticle, selectedArticle.primaryImage).alt} fill sizes="(max-width: 760px) 100vw, 760px" unoptimized /></div>{selectedArticle.galleryImages.length ? <div className="article-modal-gallery">{selectedArticle.galleryImages.map((image) => { const description = getArticleImageDescription(selectedArticle, image); return <figure key={image}><div><Image src={image} alt={description.alt} fill sizes="(max-width: 760px) 50vw, 360px" unoptimized /></div>{description.caption && <figcaption>{description.caption}</figcaption>}</figure>; })}</div> : null}{selectedArticle.body.map((section) => <section key={section.heading}><h3>{section.heading}</h3>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}<div className="post-social-derivative"><span className="eyebrow">Text pro sociální sítě</span><p>{selectedArticle.socialCopy}</p><strong>Carousel</strong><ol>{selectedArticle.carousel.map((slide) => <li key={slide}>{slide}</li>)}</ol><small>CTA: {selectedArticle.cta}</small></div></section></div>}
