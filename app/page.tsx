...
import { articleContent, articleContentBySlug, articleToMarkdown, type ArticleContent } from "./article-content";
import {
...
export default function Home() {
  ...
  return (
    <div className={`campaign-app theme-${theme}`}>
      ...
      {selectedArticle && <div className="modal-backdrop" onMouseDown={() => setSelectedArticle(null)}><section className="detail-modal article-library-modal article-readable-card" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelectedArticle(null)}>×</button><h2>{selectedArticle.title}</h2><p className="article-perex">{selectedArticle.perex}</p><div className="article-modal-actions"><button className="primary-button" onClick={() => shareArticle(selectedArticle)}>Sdílet</button><a className="secondary-button" href={articlePublicPath(selectedArticle)} target="_blank" rel="noreferrer">Otevřít na webu</a><a className="secondary-button" href={articleMarkdownUrl(selectedArticle)} target="_blank" rel="noreferrer">Otevřít Markdown</a><button className="secondary-button" onClick={() => copyToClipboard(articleFullText(selectedArticle), "Celý článek")}>Kopírovat celý článek</button><button className="secondary-button" onClick={() => copyToClipboard(selectedArticle.socialCopy, "SoMe text článku")}>Kopírovat SoMe text</button></div><div className="article-modal-media"><Image src={selectedArticle.primaryImage} alt={`Primární fotografie článku ${selectedArticle.title}`} fill sizes="(max-width: 760px) 100vw, 760px" unoptimized /></div>{selectedArticle.body.map((section) => <section key={section.heading}><h3>{section.heading}</h3>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}<div className="post-social-derivative"><span className="eyebrow">Text pro sociální sítě</span><p>{selectedArticle.socialCopy}</p><strong>Carousel</strong><ol>{selectedArticle.carousel.map((slide) => <li key={slide}>{slide}</li>)}</ol><small>CTA: {selectedArticle.cta}</small></div></section></div>}
